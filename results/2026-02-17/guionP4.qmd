...
Para datasets y dataformat:
```{bash}
# Descargar herramientas localmente si no existen
if [ ! -f ./datasets ]; then
...
```{bash}
if [ ! -f trypanosoma_annotated.txt ]; then
...
:::{.callout-important icon=false title="Respuesta1"}
Una letra en el nombre del acceso, si no me equivoco hacen referencia a 2 versiones del mismo ensamblaje, mientras que GCA es la original y pertenece al investigador, GCF ha sido curado por NCBI específicamente
:::

2. Busca una función de R que te permita identificar las líneas repetidas en un
   marco de datos.
   
:::{.callout-important icon=false title="Respuesta 2"}
```{r}
duplicados<-duplicated(genomas) 
print(duplicados)
table(duplicados)
```
Nos dice que hay 35 filas que dan FALSE en duplicado, osea que son únicas y 0 filas duplicadas
:::
...
genomas <- genomas[order(genomas$Assembly.Release.Date), ] # Ordenado por el año de lanzamiento del ensamblamiento, en ascendente
kable(genomas) %>% kable_styling() 
```
...
:::{.callout-tip icon=false title="Función"}
Esto permite atribuir a la columna de fecha de lanzamiento un valor de fecha, que no se lea como texto, sino como numero
```{r}
...
```
**Ahora que la columna de la que hablabamos se toma como una fecha podemos crear una tabla que saque por pantalla el numero de secuencias lanzadas en cada año**
...
:::{.callout-important icon=false title="Comprobar el dataframe"}
```{r}
dim(gff)
head(gff)
tail(gff)
```
Es una matriz de 42497 filas y 9 columnas, todo parece en orden.
:::
...
```
**Hemos creado 5 nuevos objetos correspondientes a cada valor de la variable type en la tabla anterior.**

:::{.callout-note icon=false title=Desafío}
...
**Para identificar en los marcos anteriores (objetos) que columnas tienen el mismo valor en todas las líneas y eliminarlas utilizaremos funciones del paquete dplyr**
```{r}
library(dplyr)
...
```
Modificamos cada objeto, no creamos nuevos, el codigo selecciona las # columnas de cada objeto y confirma si hay alguna fila que tenga valores diferentes, sino, son todos iguales, asi que lo elimina.
:::
:::

# **Hasta aquí me dio tiempo a hacer en clase, los errores posteriores se deben a que no se ha continuado la práctica**
